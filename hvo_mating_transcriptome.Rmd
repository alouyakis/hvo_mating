---
title: "Haloferax volcanii mating experiments"
...
#rm(annomap, cds_ids, gff_clean, keggmap, locus_tag, go_plus, sub.keggmap, papke)
...
rm(arcog_uri, cds_ids, gff_clean, gff_map, go_plus, keggmap)
```
...
lists <- list(
  ...
  `tivss`=c("HVO_0421", "HVO_0620", "HVO_0748", "HVO_1160", "HVO_1217", "HVO_2278", "HVO_2385", "HVO_C0036",
  "HVO_2281", "HVO_C0036")
  # `crispr_cas`=c("HVO_A0205", "HVO_A0206", "HVO_A0207", "HVO_A0208","HVO_A0209", "HVO_A0210", "HVO_A0211", "HVO_A0212"),
  # `glyc`=c("HVO_0727", "HVO_1524", "HVO_1527", "HVO_1528", "HVO_1529", "HVO_1530", "HVO_1531", "HVO_A0586",
  #                   "HVO_1517", "HVO_1523A", "HVO_0798", "HVO_1526", "HVO_1522", "HVO_1523", "HVO_2072", "HVO_1734A"),
  # `lsglyc`=c("HVO_2043", "HVO_2044", "HVO_2045", "HVO_2046", "HVO_2047", "HVO_2048", "HVO_2049",
  # "HVO_2051", "HVO_2052", "HVO_2053", "HVO_2054", "HVO_2055", "HVO_2056", "HVO_2057",
  # "HVO_2058", "HVO_2059", "HVO_2060", "HVO_2061", "HVO_2062", "HVO_2063", "HVO_2064")
)
...
for(i in 1:length(lists)){
  ...
  df_sub1 <- merge(df_sub, coords_sub, all.x = TRUE)
  df_sub1 <- merge(df_sub1, coords01_sub, all.x = TRUE)
  p <- ggplot(data=df_sub1, aes(x= paste(new, 
                                         # prot, sep = '\n'), # single line prot
                                         str_wrap(prot, width = 40), sep = '\n'), # multiline x-axis
                                y=value, fill = variable)) +
    theme_bw() +
    geom_bar(stat="identity", position = position_dodge(width = NULL), width = 0.7) +
    geom_hline(yintercept = 0) +
    ...
    labs(title = paste(nm,"genes"), 
         x = NULL, y = expression("log"["2"]*" fold change"), fill = "time vs shaking") +
    theme(text = element_text(size=12), axis.text.x = element_text(size  = 12, angle = 45, hjust = 1, vjust = 1)) +
    theme(plot.title = element_blank()) +
    theme(plot.margin = unit(c(0.5,0.5,0.5,2),"cm")) + ## use this for most plot margins
    # theme(plot.margin = unit(c(0.5,1,0,1.5),"cm")) + ## use this for cowplot for rm
    scale_fill_manual(labels = c("t00", "t02", "t04", "t08", "t24"), 
                      values = c("#000000", "#E69F00", "#009E73", "#0072B2", "#D55E00")) +
  ...
  ggsave(filename = paste("plots/bar_",nm2,"_histogram.pdf", sep = ""), 
...
p0$prot <- as.character(p0$prot)
p0[840, 9] <- "ATPase FlaH"
p0[834, 9] <- "archaellar protein FlaCE"
...
p0[839, 9] <- "archaellin FlgA2"
p0[2418, 9] <- "archaellin FlgA1"
p0[555, 9] <- "response regulator CheY"
p0[556, 9] <- "response regulator CheY"
p0[2058, 9] <- "methyl−accepting chemotaxis protein Htr39"
...
p0[2469, 9] <- "archaellin pilA2"
p0[2470, 9] <- "archaellin pilA3"
p0[2471, 9] <- "archaellin pilA4"
p0[2472, 9] <- "archaellin pilA5"
p0[2473, 9] <- "archaellin pilA6"
p0[2814, 9] <- "archaellin type IV pilin"
p0$prot <- as.factor(p0$prot)
...
flagella <- c("HVO_1033", "HVO_1034", "HVO_1160", "HVO_1200", "HVO_1203", 
              ...
              "HVO_1216", "HVO_2993", "HVO_2062", "HVO_2450", "HVO_2451", 
              "HVO_A0632", "HVO_A0633", "HVO_2704", "HVO_1160")
...
# df <- stack(mget(c('secretion_system', 'chemotaxis', 'flagella', 'eps', 'other_flagella')))
...
# ggplot(data=arrange(df_sub1,category), aes(x = paste(new, prot, sep='\n'), y=value, fill = variable)) +
    ...
    # geom_signif(stat="identity", aes(y = val, x = new, annotation = "*", xend = new, yend = val)) +
    geom_hline(yintercept = 0) +
...
# ggplot(data=arrange(df_sub1,category), aes(x = paste(new, prot, sep='\n'), y=value, fill = variable)) +
  ...
  geom_text(aes(44.5, 5, label = "IS5 Family", vjust = 1.2, hjust = 1), color = "grey47", size = 3) +
    ...
    # geom_text(aes(y = valt), label = "*", position = position_dodge(width = 0.7)) +
    # geom_text(aes(y = val01t), label = "**", position = position_dodge(width = 0.7)) +
    aes(x = fct_inorder(paste(new), category)) +
    labs(x = NULL, y = expression("log"["2"]*" fold change"), fill = "time vs shaking") +
    coord_flip()
  # facet_wrap(~category, nrow = 1, scales = "free_x")
# longplot
ggsave("plots/figS3_transposases.pdf", height = 15, width = 8)
```
...
plot_rms
viol
plot_grid(manplot, viol, plot_rms, labels = c('A', 'B', 'C'), label_size = 14, ncol = 1, nrow = 3)
ggsave("plots/figure6_selfish_genes.pdf", height = 15, width = 8, units = "in")


manplotc
plot_crispr_cas
plot_grid(manplotc, plot_crispr_cas, labels = c('A', 'B'), label_size = 14, ncol = 1, nrow = 2)
...
ggsave("plots/figure2_glycosylation.pdf", height = 18, width = 14, units = "in")
```

```{r top and bottom de}
#head(logfc)
#for(i in c("de00","de02","de04","de08","de24")) {
top_n(p0, 20, de00) %>% arrange(-de00) %>%
  write.table(file = "tables/de00_top20.txt", quote = FALSE, sep = "\t", row.names = FALSE) 
top_n(p0, 20, de02) %>% arrange(-de02) %>%
  write.table(file = "tables/de02_top20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
top_n(p0, 20, de04) %>% arrange(-de04) %>%
  write.table(file = "tables/de04_top20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
top_n(p0, 20, de08) %>% arrange(-de08) %>%
  write.table(file = "tables/de08_top20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
top_n(p0, 20, de24) %>% arrange(-de24) %>%
  write.table(file = "tables/de24_top20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
#max(logfc$de00, na.rm = TRUE)

top_n(p0, -20, de00) %>% arrange(-de00) %>%
  write.table(file = "tables/de00_bottom20.txt", quote = FALSE, sep = "\t", row.names = FALSE) 
top_n(p0, -20, de02) %>% arrange(-de02) %>%
  write.table(file = "tables/de02_bottom20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
top_n(p0, -20, de04) %>% arrange(-de04) %>%
  write.table(file = "tables/de04_bottom20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
top_n(p0, -20, de08) %>% arrange(-de08) %>%
  write.table(file = "tables/de08_bottom20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
top_n(p0, -20, de24) %>% arrange(-de24) %>%
  write.table(file = "tables/de24_bottom20.txt", quote = FALSE, sep = "\t", row.names = FALSE)
...
for(i in 1:length(lists)){
  ...
  coords01_sub <- filter(coords01, grepl(paste((lists[[i]]), collapse="|"), old))
  df_sub$Row.names <- NULL
  df_sub <- melt(df_sub, id.vars=c("old", "new", "prot", "gene_name", "arcog"))
  df_sub1 <- merge(df_sub, coords_sub, all.x = TRUE)
  df_sub1 <- merge(df_sub1, coords01_sub, all.x = TRUE)
  p <- ggplot(data=df_sub1, aes(x= paste(new, str_wrap(prot, width = 30), sep='\n'), y=value, fill = variable)) +
    theme_bw() +
    geom_bar(stat="identity", position = position_dodge(width = NULL), width = 0.7) +
    geom_hline(yintercept = 0) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
    ...
    theme(plot.title = element_text(hjust = 0.5), plot.margin = unit(c(0.5,0.5,0.5,1),"cm")) +
    scale_fill_manual(labels = c("t00", "t02", "t04", "t08", "t24"), 
                      values=c("#000000", "#E69F00", "#009E73", "#0072B2", "#D55E00")) +
    geom_text(aes(y = val), label = "*", position = position_dodge(width = 0.7), size = 2) +
    geom_text(aes(y = val01), label = "**", position = position_dodge(width = 0.7), size = 2) +
...
ggplot(cluster_all$plot$data) +
  ...
  geom_boxplot(aes(x = time_de, y = value, alpha = 0.2)) +
  geom_point(position = "jitter", aes(x = time_de, y = value, alpha = 0.2)) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(legend.position = "none") +
...
ggsave("plots/figS7_clusters.pdf", height = 20, width = 14)
# ggsave("exp_mating/plots/supp_fig_clusters.tiff")
